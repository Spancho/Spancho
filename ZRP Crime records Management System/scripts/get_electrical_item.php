<?php 
	...
	if(isset($_POST['search']))
		$rs = $dbManager->getCustomQuery("select * from electrical_item where identity like '%".$_POST['identity']."%'");
	else
		$rs = $dbManager->getCustomQuery("select * from electrical_item order by id desc limit 20");

	 ?> 
   	<script language="javascript">
        function loadDetails(id){
            location = 'index.php?page=electrical_item.php&id='+id;
        }
    </script>

     <h3>Search Stolen Electrical Items</h3> [ <a href="scripts/print_electrical_item.php" target="_blank">Print</a> ]
 <hr />
