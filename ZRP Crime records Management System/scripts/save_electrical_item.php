...
?>
	...
	if(isset($_POST["save"])){
		...
		if(empty($_REQUEST["id"]) || $_REQUEST["id"] == 0)
			$dbManager->saveSingleRecord("electrical_item", $saveArray);
		else
			$dbManager->updateSingleRecord("electrical_item", $saveArray, $_REQUEST["id"]);
		?>			
			<script language="javascript">
				$("#info").fadeIn();
				$("#info").html("Record successfully saved.");
				$("#info").fadeOut(3000);
				location = 'index.php?page=get_electrical_item.php';
			</script>
		<?php
	}
	 ?>
     <?php	
	if(!empty($_GET['id']))
		$rs = $dbManager->getCustomQuery("select * from where id = ".$_GET['id']);
