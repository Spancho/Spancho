...
?>
	...
	if(isset($_POST["save"])){
		...
		$saveArray["station"] = $_REQUEST["station"];
		$saveArray["cr_number"] = $_REQUEST["cr_number"];
		$saveArray["offence"] = $_REQUEST["offence"];
		
		
		if(empty($_GET["id"]) || $_GET["id"] == 0)
			$dbManager->saveSingleRecord("wanted_person", $saveArray);
		else
			$dbManager->updateSingleRecord("wanted_person", $saveArray, $_GET["id"]);
		?>			
			<script language="javascript">
				$("#info").fadeIn();
				$("#info").html("Record successfully saved.");
				$("#info").fadeOut(3000);
				location = 'index.php?page=get_wanted_person.php';
			</script>
		<?php
	...
	<script language="javascript">
		function validate(){
			...
			if($("#station").val() == ''){
				$("#error").fadeIn();
				$("#error").html("Please enter the station.");
				$("#error").fadeOut(3000);
				return false;
			}
...
*/
	 ...
	 <table width="450" height="189" border = "0">
	<tr>
	  <td>Station</td>
	  <td><input type = "text" name = "station" id="station" value="<?php echo $rs['station'];?>" /></td>
	</tr>
	<tr>
	  <td>CR Number</td>
	  <td><input type = "text" name = "cr_number" value="<?php echo $rs['cr_number'];?>" /></td>
	</tr>
	<tr>
	  <td>ID Number </td><td><input name="id1" type="text" id="id1" size="4" maxlength="2" class="span2" onblur="autoFill()" value="<?php echo $id1;?>" />
	    -
